// sum reduction on gpu global memory
/*
/* The code  is a CUDA implementation of parallel sum reduction using global memory. 
   It divides the work into two kernel calls: 
   the first performs the sum reduction within each block, and 
   the second reduces the intermediate results from all blocks into a final result. 

Why running the Kernel 100 times?
  This code is running the kernel 100 times (as specified by the loop with NUM_TRIALS) 
  which serves primarily for benchmarking purposes. By running the kernel multiple times, 
  you get a better understanding of how consistent the kernel's performance is.
  One-time execution could be influenced by outliers, such as initial GPU warm-up times or 
  background processes on the GPU. By running the kernel multiple times, 
  the initial overhead of starting the kernel is minimized in the average execution time.
*/
#include <iostream>
#include <iomanip>
#include <cstdlib>
...
int main( int argc, char* argv[] ) {
  ...
  */
  ...
  /* Note : the sequential and parallel sum will match only if you run the kernel once
           as the vector changes on each kernel execution
  */
  //cout << "sequential sum: " << sum << endl ;
  //cout << "parallel sum:" << h_out << endl << endl ;
